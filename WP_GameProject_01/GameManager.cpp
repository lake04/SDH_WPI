...
void Start()
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		Clouds[i] = StartCloud(ScreenDC, IDB_CLOUD);
...
void Score()
{
	...
	score++;
	wsprintf(strTemp, L"Score = %06d", score);
	TextOut(MemoryDC,400, 0, strTemp, wcslen(strTemp));
	if (score > maxScore) maxScore = score;

	wchar_t strTemp2[128] = L"";
	wsprintf(strTemp2, L"MaxScore = %06d", maxScore);
	TextOut(MemoryDC, 500, 0, strTemp2, wcslen(strTemp2));
	...
	if (score % 100 == 0)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			setSpeedX(Cactus[i], cactusSpeed);
		}
	}
}
